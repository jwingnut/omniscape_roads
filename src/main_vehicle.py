...
from generate_vehicle_raster import generate_vehicle_raster, get_regions_to_process
...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s',
                    handlers=[logging.FileHandler("omniscape_run.log"), logging.StreamHandler()])
...
def process_single_case(run_folder, args, api_key):
    city_group, expand_distance, pixel_size, conductance_raster_edge_buffer, vehicle_pair, threads, radius, block_size, case_type, region, h, w = args
    vehicle_edge_buffer, vehicle_edge_buffer_value = vehicle_pair
    ...
    try:
        ...
        logging.info(f"Graph with capacity prepared: {graph_path_with_capacity}")

        vehicle_raster_path = generate_vehicle_raster(
            city_group, pixel_size, vehicle_edge_buffer,
            vehicle_edge_buffer_value, region, case_type, case_folder, h, w, api_key
        )
        logging.info(f"Vehicle raster generated: {vehicle_raster_path}")
        ...
        conductance_raster_path = generate_conductance_raster(
            ...
            vehicle_raster_path, case_folder
        )
        logging.info(f"Conductance raster generated: {conductance_raster_path}")

        condition_raster_path = generate_condition_raster(
            city_group, pixel_size, vehicle_edge_buffer, case_folder
        )
        logging.info(f"Condition raster generated: {condition_raster_path}")

        omniscape_output_path = run_omniscape(
            city_group, vehicle_raster_path, conductance_raster_path, condition_raster_path,
            threads, radius, block_size, pixel_size, vehicle_edge_buffer,
            vehicle_edge_buffer_value, conductance_raster_edge_buffer, case_folder
        )
        ...
        if omniscape_output_path:
            ...
            save_results_to_graphml(
                ...
                vehicle_edge_buffer, vehicle_edge_buffer_value, conductance_raster_edge_buffer,
                case_folder
            )
            return case_folder
        else:
        ...
        logging.error(f"Error processing {case_type}, region: {region_str} for {city_group}: {str(e)}")
...
def main():
    ...
    parser.add_argument("--vehicle_edge_buffers", type=int, nargs="+", required=True)
    parser.add_argument("--vehicle_edge_buffer_values", type=float, nargs="+", required=True)
    ...
    vehicle_pairs = list(itertools.product(args.vehicle_edge_buffers, args.vehicle_edge_buffer_values))
    ...
    for params in itertools.product(
        ...
        city_group, expand_distance, pixel_size, conductance_raster_edge_buffer, vehicle_pair, threads, radius, block_size = params
        ...
        h, w = args.regions
        all_cases = [
            (city_group, expand_distance, pixel_size, conductance_raster_edge_buffer, vehicle_pair, threads, radius,
        ...
        for region in regions:
            all_cases.append((city_group, expand_distance, pixel_size, conductance_raster_edge_buffer, vehicle_pair,
                              threads, radius, block_size, "edge_region", region, h, w))

        logging.info(f"Processing run with {len(all_cases)} cases")

        process_func = partial(process_single_case, run_folder, api_key=args.census_api_key)
        ...
        # Log details of failed cases
        failed_cases = [case for case, result in zip(all_cases, results) if result is None]
        for failed_case in failed_cases:
            logging.error(f"Failed case: {failed_case}")
